...
import { fetchCharacterData, getCharacterInfo } from '@/lib/esi'
...
export const POST = withErrorHandling(async (request: Request) => {
  ...
  // Never trust characterId/characterOwnerHash from the client — derive them from the
  // access token itself (server-side JWT decode), otherwise a caller could link any
  // character it doesn't own by supplying an arbitrary characterId with a token whose
  // authenticated calls simply fail (and get silently swallowed) while public ESI calls
  // still succeed, letting an attacker squat on someone else's character record.
  const charInfo = await getCharacterInfo(accessToken)
  const characterId = charInfo.character_id
  const characterOwnerHash = charInfo.character_owner_hash
