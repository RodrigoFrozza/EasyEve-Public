...
import { syncActiveRattingActivitiesForCharacters } from '@/lib/activities/ratting-wallet-sync'
import { runAutoTrackingForUser } from '@/lib/activities/auto-activity-detection'
...
export const POST = withErrorHandling(async () => {
  ...
  try {
    ...
    const results = []
    const refreshedCharacterIds: number[] = []
    ...
    for (const char of user.characters) {
      try {
        ...
        refreshedCharacterIds.push(char.id)
      } catch (err: any) {
    ...
    if (refreshedCharacterIds.length > 0) {
      void runAutoTrackingForUser(user.id).catch((err) => {
        logger.error('Characters', 'Post-refresh auto-tracking detection failed', err)
      })
      void syncActiveRattingActivitiesForCharacters(user.id, refreshedCharacterIds).catch((err) => {
        logger.error('Characters', 'Post-refresh ratting activity sync failed', err)
      })
    }
