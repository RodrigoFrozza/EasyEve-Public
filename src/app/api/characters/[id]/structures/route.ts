...
import { getStructureInfo, getStructureNamesBatch } from '@/lib/esi'
...
export const GET = withErrorHandling(
  withAuth(async (request, user, { params }: { params: Promise<{ id: string }> }) => {
    ...
    console.log(`[Structures API] getCharacterAssets returned: ${assets?.length || 0} items`)
    ...
    const limitedLocations = uniqueLocations.slice(0, 200)
    const nameMap = await getStructureNamesBatch(limitedLocations, characterId)

    const structures = await Promise.all(
      limitedLocations.map(async (locId) => {
        const structureInfo = await getStructureInfo(locId, characterId).catch(() => null)
        const name = structureInfo?.name || nameMap.get(locId) || `Structure ${locId}`
        return {
          id: locId,
          name,
