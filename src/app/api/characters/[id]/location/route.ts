...
const LOCATION_CACHE_TTL_MS = 8_000
const locationCache = new Map<
  number,
  { payload: Awaited<ReturnType<typeof getCharacterLocation>>; expiresAt: number }
>()
...
export const GET = withErrorHandling(withAuth(async (request, user, { params }: { params: Promise<{ id: string }> }) => {
  ...
  const cached = locationCache.get(characterId)
