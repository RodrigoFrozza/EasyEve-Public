...
import { validateCharacterTags } from '@/constants/character-tags'
...
export async function POST(
  ...
  try {
    ...
    if (!validateCharacterTags(tags)) {
      return NextResponse.json(
        { error: 'Tags must be an array of strings (max 20 tags, 30 chars each)' },
        { status: 400 }
      )
    }
