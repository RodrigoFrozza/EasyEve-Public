...
import { initializeLootSnapshot } from '@/lib/activities/container-loot-sync'
...
import { maybeRunStaleAuditForUser } from '@/lib/activities/auto-activity-detection'
import { z } from 'zod'
...
export const GET = withErrorHandling(async (request: Request) => {
  ...
  await maybeRunStaleAuditForUser(user.id).catch((err) => {
    logger.warn('API/activities', 'Stale activity audit failed (non-blocking)', err)
  })
...
export const POST = withErrorHandling(async (request: Request) => {
  ...
  // Serialize concurrent creates for this user with a row lock so the free-tier
  // limit and busy-character checks can't be raced (TOCTOU) into launching two
  // activities at once.
  const activity = await prisma.$transaction(async (tx) => {
    await tx.$executeRaw(Prisma.sql`SELECT id FROM "User" WHERE id = ${user.id} FOR UPDATE`)

    const activeActivities = await tx.activity.findMany({
      where: {
        userId: user.id,
        status: 'active'
      }
    ...
    const launchNow = new Date().toISOString()
    const activityData = {
      ...(extraData as Record<string, unknown>),
      lastDataAt: launchNow,
      lastSyncAt: launchNow,
    }
  ...
  if (typeLower === 'mining') {
    try {
      ...
      logger.error('ACTIVITY', 'Initial mining sync failed', e)
    }
  }

  if (typeLower === 'ratting' || typeLower === 'escalations' || typeLower === 'exploration' || typeLower === 'salvaging') {
    void (async () => {
      try {
        if (typeLower === 'ratting' || typeLower === 'escalations') {
          await syncRattingWalletForActivity(activity)
        }

        const activityData = activity.data as Record<string, unknown> | null
        if (
          activityData?.autoLootTrackingEnabled &&
          (typeLower === 'ratting' || typeLower === 'exploration' || typeLower === 'salvaging')
        ) {
          const refreshed = await prisma.activity.findUnique({ where: { id: activity.id } })
          if (refreshed) {
            await initializeLootSnapshot(refreshed)
          }
        }
      } catch (e) {
