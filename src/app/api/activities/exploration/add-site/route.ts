...
export const POST = withErrorHandling(async (request: Request) => {
  ...
  const { activityId, siteName, spaceType, fullCargoAfter, beforeCargo } = body
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (activity.type !== 'exploration') {
      throw new AppError(
        ...
        'Only exploration activities are supported by this endpoint',
        400
      )
    }
    ...
    const beforeCargoText =
      beforeCargo !== undefined
        ? beforeCargo
        : String(activityData.lastCargoState || '')
    const loot = calculateLootDelta(beforeCargoText, fullCargoAfter)

    if (loot.length === 0) {
      throw new AppError(
        ErrorCodes.VALIDATION_FAILED,
        'No new items detected in cargo delta',
        400
      )
    }
    ...
    const updatedData = {
      ...activityData,
      totalLootValue: (Number(activityData.totalLootValue) || 0) + addedValue,
      currentCargoValue: (Number(activityData.currentCargoValue) || 0) + addedValue,
