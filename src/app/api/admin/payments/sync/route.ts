...
export const POST = withErrorHandling(withAuth({ requiredRole: 'master' }, async () => {
  ...
  for (const entry of journal) {
    ...
    // Try to find user by Account Code (for alts/unregistered characters). The
    // payer's typed note lives in `reason`; `description` is CCP's auto text.
    ...
    // Create payment, credit ISK, and record history atomically — a mid-way
    // crash must never leave a payment without its balance credit (or vice versa).
