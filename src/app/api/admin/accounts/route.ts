 ...
 */
export const DELETE = withErrorHandling(withAuth({ requiredRole: 'master' }, async (request: Request, admin) => {
  ...
  if (userId === admin.id) {
    throw new AppError(ErrorCodes.VALIDATION_FAILED, 'You cannot delete your own account', 400)
  }

  const target = await prisma.user.findUnique({
    where: { id: userId },
    select: { accountCode: true, role: true },
  })

  if (!target) {
    throw new AppError(ErrorCodes.API_NOT_FOUND, 'Account not found', 404)
  }
  ...
  await prisma.securityEvent.create({
    data: {
      ...
      details: {
        ...
        deletedAccountCode: target.accountCode,
        deletedRole: target.role,
      },
    },
  })

  logger.info('Admin:Accounts', `Account ${userId} deleted by master ${admin.id}`)
