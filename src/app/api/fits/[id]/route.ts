...
import { CreateFittingSchema, resolveFitShipTypeId } from '@/lib/schemas'
import { AppError } from '@/lib/app-error'
import { ErrorCodes } from '@/lib/error-codes'
import { assertFittingIsValid } from '@/lib/fits/fitting-validation-service'
...
export const PUT = withErrorHandling(async (
  ...
  // Normalize the legacy `shipId` alias into `shipTypeId` (see #5.1 in
  // AUDIT_REPORT_2026-07-05.md) so a stale client bundle still persists the ship.
  const shipTypeId = resolveFitShipTypeId(body)

  // Validate the resulting fit (existing state merged with this patch), not just the
  // patch fields — otherwise a PUT that only renames the fit would skip validation
  // entirely while an unrelated earlier bug already saved an invalid module layout.
  await assertFittingIsValid({
    shipTypeId: shipTypeId ?? existing.shipTypeId,
    modules: (body.modules ?? existing.modules) as unknown[],
  })
