...
export async function GET(request: Request, { params }: RouteParams) {
  try {
    ...
    const showWallet = user.profile?.showWallet ?? true
    const showLocation = user.profile?.showLocation ?? true
    ...
    const charactersWithCorp = characters.map((char) => {
      ...
      return {
        ...char,
        walletBalance: showWallet ? char.walletBalance : null,
        location: showLocation ? char.location : null,
        corporation,
        alliance,
      }
    })
