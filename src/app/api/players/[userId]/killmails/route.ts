...
const KILLMAIL_DETAIL_CACHE_TTL = 7 * 24 * 60 * 60 * 1000 // killmails are immutable once created
...
export async function GET(request: Request, { params }: RouteParams) {
  try {
    ...
    // Resolve a valid (decrypted, refreshed-if-needed) token — tokens are stored
    // encrypted at rest, so the raw column value can't be used as a bearer.
    const { accessToken } = await getValidAccessToken(mainCharacter.id)
    if (!accessToken) {
      return NextResponse.json({ killmails: [], total: 0, page, pageSize })
    }
    ...
    const enrichedKillmails = await Promise.all(
      allKillmails.map(async (km) => {
        // Killmail details never change once published — cache indefinitely (long TTL) so
        // repeated page views (by this viewer or others) don't re-hit ESI for the same kill.
        const detail = await withCache(
          `killmail-detail:${km.killmail_id}:${km.killmail_hash}`,
          () => getKillmailDetail(km.killmail_id, km.killmail_hash),
          KILLMAIL_DETAIL_CACHE_TTL
        )
