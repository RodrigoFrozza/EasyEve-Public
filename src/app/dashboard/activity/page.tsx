...
function ActivityTrackerContent() {
  ...
  }, [viewId, activities.length]) // Only run once or when viewId/activities change
  ...
  useEffect(() => {
    ...
    setRattingContainerValidation('checking')
    const timer = window.setTimeout(() => {
      if (!newActivity.data?.autoLootContainerName) {
        setRattingContainerValidation('invalid')
        return
      }
  ...
  const totalQuantity = useMemo(
    () => activities.filter((a) => a.type === 'mining').reduce((sum, a) => sum + (Number(a.data?.quantity) || 0), 0),
    [activities]
  )
...
<div className="lg:col-span-3 p-6 space-y-8">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2" data-tour="activity-type-selector" role="radiogroup" aria-label="Activity type selection">
                        ...
                        <div className="space-y-2 col-span-1 sm:col-span-2">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              ...
                              {!autoLootFeatureEnabled && (
                                <Badge variant="outline" className="text-[8px] px-1 py-0 h-3 leading-none bg-red-500/10 text-red-400 border-red-500/20 font-black uppercase">Disabled</Badge>
                              )}
                              {autoLootFeatureEnabled && (
                                <>
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <Switch
                                disabled={!autoLootFeatureEnabled}
                                checked={!!newActivity.data?.autoLootTrackingEnabled}
                                onCheckedChange={(checked) => {
                                  if (!autoLootFeatureEnabled) return
                                  markRattingConfigInteraction()
                                  updateData({
                            ...
                            <div className={cn(
                              ...
                              <div className="space-y-1">
                                ...
                                <StructureCombobox
                                  ...
                                  disabled={!autoLootFeatureEnabled || !newActivity.data?.autoLootCharacterId}
                                />
                              </div>
                              <div className="space-y-1">
                              ...
                              <p className={cn(
                                ...
                                {rattingContainerValidation === 'checking' && t('common.loading')}
                                {rattingContainerValidation === 'valid' && `${t('activity.trackingLabel')}: ${newActivity.data?.autoLootContainerName || t('common.unknown')}`}
                                {rattingContainerValidation === 'invalid' && t('activity.noContainersFound')}
                                {rattingContainerValidation === 'idle' && t('activity.selectStructureContainerHint')}
                              </p>
                            </div>
                          )}
                        </div>
            ...
            <div className="bg-zinc-950 p-6 border-t border-eve-border/10 flex items-center justify-between">
              ...
              <Button 
                ...
                disabled={!launchValidation.isValid || isLaunching}
