...
export default async function DashboardPage() {
  ...
  const promoBanners = await getEligiblePromoBannerViewsForUser(session.user.id).catch(() => [])
    ...
    <div className="space-y-6 p-4 md:p-8 max-w-[1600px] mx-auto w-full">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-[12.5px] text-ta-muted">{t('dashboard.commandCenterSubtitle')}</p>
        </div>
        <UTCClock />
      </div>
      ...
      >
        <UserOverview
          ...
          mainCharacter={mainCharacter ? {
            ...
            system: mainCharacter.location,
            ship: mainCharacter.ship,
        ...
        <ActivityPanel activities={(userActivities || []).map(a => ({
          ...
          isPaused: a.isPaused,
          pausedAt: a.pausedAt,
          accumulatedPausedTime: a.accumulatedPausedTime,
