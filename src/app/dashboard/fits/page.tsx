...
import { Plus, Search, Filter, Rocket, Import, Globe2, Lock } from 'lucide-react'
...
export default function FitsDashboardPage() {
  ...
  const [loading, setLoading] = useState(hasPremium)
  ...
  return (
    <div className="mx-auto max-w-[1600px] space-y-6 p-6 md:p-8 animate-in fade-in duration-700">
        ...
        <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
          <div className="space-y-1.5">
            ...
            <p className="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-muted-foreground">
              <span>{t('global.manageShareFits')}</span>
              {fits.length > 0 && (
                <>
                  <span className="text-border">·</span>
                  <span className="font-medium text-foreground">{t('fits.fitsCountSummary', { count: fits.length })}</span>
                  {publicFitsCount > 0 && (
                    <span className="inline-flex items-center gap-1 text-emerald-500">
                      <Globe2 className="h-3.5 w-3.5" />
                      {t('fits.publicCountSummary', { count: publicFitsCount })}
                    </span>
                  )}
                </>
              )}
          ...
          <div className="flex w-full flex-col gap-2 sm:flex-row md:w-auto">
            ...
            <Button 
              ...
              disabled={!hasPremium}
              className={cn(
                "h-10 px-5 font-semibold transition-all",
                hasPremium ? "bg-primary text-primary-foreground hover:bg-primary/90" : "bg-muted text-muted-foreground opacity-50 cursor-not-allowed"
              )}
            >
              {hasPremium ? (
      ...
      {!hasPremium ? (
        <section className="flex flex-col items-center justify-center rounded-2xl border border-primary/20 bg-primary/5 px-6 py-20 text-center backdrop-blur-sm animate-in zoom-in-95 duration-500">
           ...
           <h2 className="mt-6 text-2xl font-bold tracking-tight text-foreground">
             {t('premium.premiumRequired') || 'Premium Access Required'}
           </h2>
           <p className="mt-2 max-w-md text-muted-foreground leading-relaxed">
             {t('fits.premiumNotice') || 'Fit Management is exclusive to Premium subscribers. Track your ship configurations, analyze performance, and share them with your fleet.'}
           </p>
           <Button asChild className="mt-8 h-11 px-10 font-bold shadow-xl shadow-primary/20 transition-all hover:scale-105 hover:shadow-primary/30">
             <Link href="/dashboard/subscription">
               {t('global.upgradeToPremium') || 'Upgrade to Premium'}
             </Link>
           </Button>
        </section>
