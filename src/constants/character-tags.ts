...
/** One-click presets shown on character cards and list rows */
export const CHARACTER_TAG_QUICK_ADD = ['Ratter', 'Miner', 'Alt'] as const
...
/**
 * "Teal Aurora" activity → color (hex) for tag pills, group dots and sparklines.
 * Mirrors the design system §2 activity map; app tags are folded into the
 * closest activity family. Unmapped tags fall back to the neutral pill.
 */
export const CHARACTER_TAG_NEUTRAL_COLOR = '#b9c6d2' as const

export const CHARACTER_TAG_COLOR: Record<string, string> = {
  Miner: '#34b3a4', // mining — teal
  Industrialist: '#34b3a4', // industry — teal family
  Ratter: '#e0a94a', // ratting — amber
  Hauler: '#6ea8fe', // logistics — blue
  Explorer: '#6ea8fe', // exploration — blue
  'Planet Manager (PI)': '#8ad06a', // passive income — green
  'Abyssal Runner': '#a98bfa', // abyssal — violet
  'Capital Pilot': '#a98bfa', // special — violet
  PvPer: '#f47272', // pvp — red
  'Skill Point Farmer': '#8a9aa8', // neutral-ish
  Alt: CHARACTER_TAG_NEUTRAL_COLOR,
}

export function getCharacterTagColor(tag: string): string {
  return CHARACTER_TAG_COLOR[tag] ?? CHARACTER_TAG_NEUTRAL_COLOR
}
