...
export function ActivityCard({ activity, index, onEnd }: ActivityCardProps) {
  ...
  const handleSyncFinancials = async () => {
    ...
    if (isExploration || isAbyssal) {
      ...
      try {
        ...
        const data = { ...(activity.data as any), totalLootValue: newTotalValue, logs: updatedLogs }
  ...
  return (
    <TooltipProvider delayDuration={200}>
      <Card className="bg-zinc-950/40 backdrop-blur-xl border border-white/5 rounded-[40px] overflow-hidden shadow-2xl transition-all duration-700 hover:border-blue-500/20 group/card relative">
        <div className="absolute inset-0 bg-gradient-to-br from-blue-500/5 via-transparent to-transparent opacity-0 group-hover/card:opacity-100 transition-opacity duration-1000 pointer-events-none" />
