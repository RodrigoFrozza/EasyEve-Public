...
export function RattingSummaryPanel({ activity, logs, viewMode, onOpenMTU }: RattingSummaryPanelProps) {
  ...
  const characterData = useMemo(() => {
    ...
    const m = getActivityFinancialMetrics(activity)
    return [{
      charId: -1,
      charName: t('activity.summary.fleetAggregate'),
      bounty: m.bounties + m.additionalBounties,
      ess: m.ess,
      tax: m.taxes,
      total: m.net,
      logs: [] as RattingLogEntry[],
    }]
  }, [logs, t, activity])
  ...
  const grossPool = metrics.bounties + metrics.ess + metrics.additionalBounties
  const netTotal = metrics.net
  ...
  return (
    <div className="space-y-16">
      ...
      >
        <ActivityStatDisplay
          label={t('activity.summary.pilotNet')}
          value={netTotal}
          variant="accent"
          formatAsNumber
          subValue="ISK"
        ...
        />
        <ActivityStatDisplay
          label={t('activity.summary.grossBounty')}
          value={grossPool}
          variant="success"
          formatAsNumber
          subValue="ISK"
        ...
        />
        <ActivityStatDisplay
          label={t('activity.summary.totalTax')}
          value={metrics.taxes}
          variant="warning"
          formatAsNumber
          subValue="ISK"
      ...
      >
        ...
        <div className="relative w-full xl:w-[450px] group z-10">
          ...
          <Input 
            placeholder={t('activity.summary.filterCombatPersonnel')}
            value={searchTerm}
      ...
      <div className="space-y-8">
        <div className="flex items-center gap-6 mb-4">
           <h4 className="text-[11px] uppercase font-black tracking-[0.5em] text-zinc-700 font-outfit shrink-0">{t('activity.summary.combatPersonnelLogs')}</h4>
        ...
        {filteredData.length === 0 ? (
          ...
          >
            ...
            <p className="text-[11px] text-zinc-600 font-black uppercase tracking-[0.5em] font-outfit">{t('activity.summary.noCombatData')}</p>
          </motion.div>
        ) : (
              ...
              >
                ...
                <div className="flex items-center gap-6 xl:justify-end">
                  <div className="text-left xl:text-right">
                    <p className="text-[10px] text-zinc-600 font-black uppercase tracking-[0.3em] font-outfit mb-1">{t('activity.summary.individualNet')}</p>
                ...
                {(viewMode === 'detailed' || expandedChars[group.charId]) && (
                  ...
                  >
                    <div className="p-8 sm:p-12 space-y-3 max-h-[500px] overflow-y-auto custom-scrollbar">
                      {group.logs.length === 0 ? (
                        <p className="text-center text-[11px] text-zinc-500 font-outfit uppercase tracking-widest py-8">
                            ...
                            <div className="flex items-center gap-6">
                              ...
                              <div>
                                <p className="text-[11px] font-black uppercase text-zinc-300 tracking-[0.2em] font-outfit group-hover/log:text-white transition-colors">
                                  {log.type === 'bounty'
                                    ? t('activity.summary.bountyProtocol')
                                    : log.type === 'ess'
                                      ? t('activity.summary.essProtocol')
                                      : t('activity.summary.taxProtocol')}
                                </p>
