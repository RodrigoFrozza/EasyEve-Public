...
import { ActivityEnhanced, AbyssalActivityData, AbyssalLogEntry, isAbyssalActivity } from '@/types/domain'
...
export function AbyssalSummaryPanel({ activity, viewMode }: AbyssalSummaryPanelProps) {
  ...
  const characterData = useMemo(() => {
    ...
    logs.forEach(log => {
      ...
      groups[charId].totalValue += log.amount || 0
      groups[charId].itemsCount += 1
  ...
  const totalLootValue = metrics.net
  
  const totalItems = lootContents.reduce((acc: number, item: { quantity?: number }) => acc + (item.quantity || 0), 0)
  
  const tier = activity.data.lastRunDefaults?.tier || activity.data.tier || '?'
  const runsCompleted = (abyssalData.runs || []).filter((r) => r.status === 'completed').length
  const pilotsCount = activity.participants?.length || 0
  const runs = abyssalData.runs || []
    ...
    <div className="space-y-16">
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-8 mb-12 relative z-10">
             <div className="flex items-center gap-6">
                ...
                <div>
                   <h3 className="text-[11px] font-black uppercase tracking-[0.4em] text-zinc-400 font-outfit">{t('activity.summary.abyssalLootCache')}</h3>
                   <p className="text-[11px] text-zinc-600 font-bold uppercase mt-2 font-outfit tracking-widest">{t('activity.summary.abyssalLootSub')}</p>
                </div>
             </div>
             <div className="bg-black/40 px-6 py-3 rounded-xl border border-white/5 backdrop-blur-md">
                <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest font-outfit">{t('activity.summary.totalItems')}: {formatNumber(totalItems)}</span>
             </div>
          </div>
      ...
      {runs.length > 0 && (
        <motion.div
          ...
          className="bg-zinc-950/40 border border-white/5 rounded-[32px] p-6 sm:p-8 backdrop-blur-md shadow-xl space-y-4"
        >
          <h3 className="text-[11px] font-black uppercase tracking-[0.35em] text-zinc-400 font-outfit">{t('activity.summary.runHistory')}</h3>
          <div className="space-y-2 max-h-[280px] overflow-y-auto custom-scrollbar">
            {[...runs].reverse().map((run) => (
              <div
                key={run.id}
                className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 rounded-2xl bg-black/30 border border-white/5 text-[11px]"
              >
                <span className="font-mono text-zinc-500 truncate">{run.id.slice(0, 8)}</span>
                <span className="text-zinc-400">{t('activity.summary.runTier')}: {run.tier ?? '—'}</span>
                <span className={cn(
                  'uppercase font-black font-outfit',
                  run.status === 'completed' ? 'text-emerald-400' : run.status === 'death' ? 'text-rose-400' : 'text-amber-400'
                )}>
                  {t('activity.summary.runStatus')}: {run.status}
                </span>
                <span className="font-mono text-white tabular-nums">{formatISK(run.lootValue || 0)}</span>
              </div>
            ))}
          </div>
        </motion.div>
      ...
      >
        ...
        <Input 
          placeholder={t('activity.summary.filterRunPersonnel')}
          value={searchTerm}
      ...
      <div className="space-y-4">
        {filteredData.length === 0 ? (
          ...
          >
            ...
            <p className="text-xs text-zinc-600 font-black uppercase tracking-[0.4em] font-outfit">{t('activity.summary.noPersonnelLogs')}</p>
          </motion.div>
        ) : (
              ...
              >
                <div className="flex items-center gap-8">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                       <span className="text-[10px] font-black uppercase tracking-widest text-zinc-500 bg-white/5 px-3 py-1 rounded-full border border-white/5">{t('activity.summary.abyssalRunner')}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-6">
                  <div className="text-right">
                    <p className="text-[10px] text-zinc-500 font-black uppercase tracking-[0.3em] mb-1 font-outfit">{t('activity.summary.pilotNetValue')}</p>
                    ...
                    <p className="text-[10px] text-zinc-600 mt-1 font-outfit">{group.logs.length} {t('activity.summary.runLogEntries')}</p>
                  </div>
                  <Button
                    variant="ghost"
                    size="icon"
                ...
                {(viewMode === 'detailed' || expandedChars[group.charId]) && (
                  ...
                  >
                    <div className="p-6 space-y-3 max-h-[500px] overflow-y-auto custom-scrollbar">
                      {group.logs.length === 0 ? (
                        <p className="text-center text-[11px] text-zinc-500 font-outfit uppercase tracking-widest py-8">{t('common.session.noLogs')}</p>
                      ) : (
                      <div className="grid grid-cols-1 gap-2">
                        {group.logs.map((log, logIdx) => (
                          ...
                          >
                            <div className="flex items-center gap-5">
                              ...
                              <div>
                                <p className="text-[11px] font-black uppercase text-white/90 tracking-widest font-outfit">
                                  {t('activity.summary.abyssalTrace')}
                                </p>
                            ...
                            <div className="text-right">
                              <p className="text-lg font-black font-mono text-emerald-400 tabular-nums">{formatISK(log.amount || 0)}</p>
                              <div className="text-[9px] text-zinc-600 font-black uppercase tracking-tighter mt-1">{t('activity.summary.loggedEntry')}</div>
                            </div>
                          </div>
                        ))}
                      </div>
