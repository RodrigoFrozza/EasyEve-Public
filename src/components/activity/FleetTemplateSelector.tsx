...
export function FleetTemplateSelector({
  ...
  const handleTemplateChange = (value: string) => {
    ...
    if (value.startsWith('preset:')) {
      const preset = presetsByTag.find(p => p.tag === value.replace('preset:', ''))
      if (preset) {
  ...
  const currentValue = selectedTemplate === 'preset' ? 'preset' : (selectedTemplate || '')
            ...
            {selectedTemplate && selectedTemplate !== 'preset' && (
              <div className="flex items-center gap-1">
                {customTemplates.find(t => t.id === selectedTemplate) && (
                  <>
                    <button
                      onClick={(e) => handleOpenEditDialog(customTemplates.find(t => t.id === selectedTemplate)!, e as any)}
                      className="p-1 text-zinc-500 hover:text-white"
                    >
                      <Edit2 className="h-3 w-3" />
                    </button>
                    <button
                      onClick={(e) => handleDeleteTemplate(customTemplates.find(t => t.id === selectedTemplate)!, e as any)}
