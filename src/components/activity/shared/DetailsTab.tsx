...
export function DetailsTab({ activity, logs = [] }: DetailsTabProps) {
  ...
  const data = useMemo(
    () => (activity.data || {}) as Partial<ActivityData>,
    [activity.data]
  )
  ...
  const topItems = useMemo(() => {
    ...
    if (isMiningActivity(activity)) {
      ...
      lootContents.forEach((item: { name?: string; value?: number; quantity?: number }) => {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <ActivityStatDisplay
              ...
              value={
                logs
                  .filter((l: { type?: string }) => l.type === 'bounty')
                  .reduce((sum: number, l: { amount?: number }) => sum + (Number(l.amount) || 0), 0) /
                Math.max(1, logs.filter((l: { type?: string }) => l.type === 'bounty').length)
              }
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <ActivityStatDisplay
              ...
              value={(data.runs || []).filter((r: any) => isAbyssalRunCompleted(r)).length}
      ...
      {isSalvagingActivity(activity) && (
        <div className="space-y-3">
          <h3 className="text-[10px] font-black uppercase tracking-widest text-zinc-500">
            {t('common.session.salvagingDetails')}
          </h3>

          <div className="grid grid-cols-2 gap-3">
            <ActivityStatDisplay
              label={t('activity.salvaging.batchesCompleted')}
              value={getSalvagingBatchCount(data.logs || [])}
              icon={<MapPin className="h-4 w-4" />}
              variant="accent"
              formatAsNumber
              size="compact"
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {topItems.map((item, idx) => (
              <div 
                key={idx}
                className="flex items-center justify-between px-4 py-3 bg-black border border-zinc-900 rounded-none"
              >
