...
import { AddExplorationLootModal, ConfirmEndModal, SiteSafetyModal } from '../modals'
...
export function ExplorationActivityContent({ 
  ...
  const top3Rewards = useMemo(() => {
    return (logs || []).filter((l: any) => l.type === 'loot')
      .sort((a: any, b: any) => {
    ...
    <div className="bg-zinc-950/40 border border-white/[0.03] rounded-xl p-3">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          <p className="text-[8px] text-zinc-500 uppercase font-black tracking-wider">Latest Discoveries</p>
          <div className="flex items-center gap-1.5 px-1.5 py-0.5 rounded-full bg-zinc-900/50 border border-white/[0.03]">
            {localIsSyncing || isSyncing ? (
              <>
                <Loader2 className="h-2 w-2 text-cyan-400 animate-spin" />
                <span className="text-[7px] text-cyan-500 font-bold uppercase tracking-tighter">Syncing...</span>
              </>
            ) : syncStatus === 'error' ? (
              <>
                <CloudOff className="h-2 w-2 text-red-500" />
                <span className="text-[7px] text-red-500 font-bold uppercase tracking-tighter">Cloud Off</span>
              </>
            ) : (
              <>
                <Cloud className="h-2 w-2 text-emerald-500" />
                <span className="text-[7px] text-emerald-500 font-bold uppercase tracking-tighter">Cloud</span>
              </>
            )}
          </div>
        </div>
    ...
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <Tooltip>
              ...
              <TooltipContent>Add Site Loot</TooltipContent>
            </Tooltip>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
        <ActivityStatDisplay
          ...
          value={activity.data?.currentSystem || t('activity.exploration.exploring')}
          subValue={activity.data?.currentSpaceType || t('activity.exploration.activeSearch')}
          variant="warning"
          size="compact"
        ...
        <ActivityStatDisplay
          ...
          value={lootContents.reduce((acc: number, item: any) => acc + (item.quantity || 0), 0).toString()}
        ...
        <ActivityStatDisplay
          ...
          value={formatISK(activity.data?.totalLootValue || 0)}
          subValue="isk"
          variant="default"
          size="compact"
          valueClassName="text-white"
        ...
        <ActivityStatDisplay
          ...
          value={formatISK(metrics.iskPerHour)}
          subValue="isk/h"
          variant="default"
          size="compact"
          valueClassName="text-white"
