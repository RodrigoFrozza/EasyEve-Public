      ...
      <DialogContent className="max-w-3xl bg-zinc-950 border-zinc-800 text-white max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[10px]">
          ...
          <div className="rounded-lg border border-zinc-800 bg-zinc-900/40 p-2">
            ...
            <p className="font-mono text-cyan-300">{topMinerVol ? formatNumber(Math.round(topMinerVol.vol)) : '0'} m³</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 p-3 lg:col-span-2">
            <p className="text-[10px] uppercase text-zinc-500 font-bold mb-2">Cumulative ISK (line)</p>
            <Sparkline data={timeline.length ? timeline : [0]} width={560} height={120} color="#fbbf24" />
          </div>
              ...
              {pieOre.map((slice) => (
                <div key={slice.label}>
                  ...
                  <div className="h-2 rounded bg-zinc-800 overflow-hidden">
                    <div className="h-full bg-amber-500" style={{ width: `${Math.min(100, slice.pct)}%` }} />
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div className="space-y-1">
              {byCharacter.map((c) => (
                <div key={c.id} className="flex items-center justify-between text-xs gap-2">
                  ...
                  <span className="font-mono text-zinc-500 shrink-0">{formatNumber(Math.round(c.vol))} m³</span>
                  <span className="font-mono text-emerald-400 shrink-0">{formatISK(c.isk)}</span>
                </div>
              ))}
            </div>
          </div>
          <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 p-3 max-h-[200px] overflow-y-auto">
            ...
            <div className="space-y-1">
              {byOre.map((o) => (
                <div key={o.typeId} className="flex items-center justify-between text-xs gap-2">
                  <span className="text-zinc-300 truncate">{o.name}</span>
                  <span className="font-mono text-zinc-500 shrink-0">{o.qty.toLocaleString()} u</span>
                  <span className="font-mono text-amber-200 shrink-0">{formatISK(o.isk)}</span>
                </div>
              ))}
            </div>
          </div>
