...
import { Package, Save, X, Plus, Loader2, TrendingUp, Circle, Info } from 'lucide-react'
...
function MTUItemRow({ item, index, onRemove }: { item: MTUItem; index: number; onRemove: () => void }) {
  return (
    <motion.div 
      ...
      className="flex items-center justify-between text-[10px] py-1.5 px-2 rounded hover:bg-zinc-800/50 group transition-all"
    >
      <div className="flex items-center gap-2 flex-1 min-w-0">
        ...
        <input
          ...
          value={item.quantity}
          onChange={(e) => {}}
          className="w-14 h-6 bg-zinc-900 border border-zinc-800 rounded text-center font-mono text-[10px] text-zinc-300 focus:border-blue-500 focus:outline-none"
          disabled
        />
...
export function MTUInputModal({ open, onOpenChange, onSave, existingItems = [] }: MTUInputModalProps) {
  const { t } = useTranslations()
  const [items, setItems] = useState<MTUItem[]>([])
  ...
  const fetchPrices = async (itemsToValuate: MTUItem[]) => {
  ...
  const handleParseRawInput = () => {
    ...
    const parsed: MTUItem[] = Array.from(parsedMap.entries()).map(([name, quantity]) => ({
      name,
      quantity,
      value: 0
    }))
    ...
    if (parsed.length === 0) {
      toast.error('Could not parse MTU content. Ensure it is in EVE clipboard format.')
      return
    }
        ...
        >
          ...
          <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <div className="space-y-3">
                <Label className="text-[10px] font-black uppercase text-blue-500 tracking-widest flex items-center gap-2">
                  <Plus className="h-3 w-3" /> {t('activity.mtu.pasteLoot')}
                </Label>
                <Textarea 
                  placeholder="1 x Kinetic Armor plating&#10;5 x Rookie ship&#10;10 x Shrouded Weapon"
                  className="min-h-[200px] bg-zinc-900/50 border border-blue-500/30 text-xs font-mono resize-none focus:border-blue-500 focus:shadow-[0_0_15px_rgba(0,191,255,0.15)] transition-all"
                  value={rawInput}
                  onChange={(e) => {
                ...
                <Button 
                  ...
                  className="w-full bg-blue-600 hover:bg-blue-500 text-white font-black uppercase text-xs tracking-widest rounded-xl transition-all hover:scale-[1.01] active:scale-[0.99]"
                >
                  {isValuating ? (
                  ...
                  {isValuating ? t('activity.mtu.pricing') : t('activity.mtu.parseItems')}
                </Button>
              </div>
                ...
                {items.length > 0 && (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <Label className="text-[10px] font-black uppercase text-blue-500 tracking-wider flex items-center gap-2">
                        <Circle className="h-2 w-2 fill-blue-500" /> {t('activity.mtu.items')} ({items.length})
                      </Label>
                      {isValuating ? (
                        <div className="flex items-center gap-2">
                          <Loader2 className="h-3 w-3 animate-spin text-blue-400" />
                          <span className="text-[9px] text-zinc-500">{t('activity.mtu.pricing')}</span>
                        </div>
                      ) : (
              ...
              >
                <div className="flex items-center gap-2 text-zinc-400">
                  ...
                  <span className="text-[10px] font-medium">{t('activity.mtu.inGameInstructions')}</span>
                </div>
              </motion.div>
            </div>
            ...
            >
              <div className="text-center py-8">
                ...
                <p className="text-xs text-zinc-500 font-medium">{t('activity.mtu.currentValue')}</p>
                <motion.p 
                  className="text-2xl font-black text-blue-400 font-mono mt-1"
                  key={totalValue}
                  initial={{ scale: 0.8 }}
                  animate={{ scale: 1 }}
                  transition={{ type: 'spring', stiffness: 500, damping: 30 }}
                ...
                {items.length > 0 && (
                  ...
                  >
                    <p className="text-[9px] text-zinc-600 font-bold uppercase tracking-widest">
                      {items.length} {t('activity.mtu.uniqueItems')}
                    </p>
                  </motion.div>
                )}
              </div>
          ...
          <DialogFooter className="p-6 bg-zinc-950/50 border-t border-white/[0.03] gap-3">
            ...
            <Button 
              ...
              className="px-8 bg-blue-600 hover:bg-blue-500 text-white font-black uppercase text-xs tracking-widest rounded-xl transition-all hover:scale-105 shadow-[0_0_20px_rgba(0,191,255,0.2)]"
            >
              <Save className="h-4 w-4 mr-2" />
              {t('activity.mtu.save')} {items.length > 0 ? `(${items.length})` : ''}
            </Button>
          </DialogFooter>
        </motion.div>
      </DialogContent>
