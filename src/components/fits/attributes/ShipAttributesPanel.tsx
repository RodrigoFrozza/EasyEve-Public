...
interface ShipAttributesPanelProps {
  ...
  calculating?: boolean
  moduleCount?: number
  /** typeIds of fitted modules, used to price the fit from real market data. */
...
export const ShipAttributesPanel: React.FC<ShipAttributesPanelProps> = ({
  ...
  const fetchPricing = useCallback(async () => {
    ...
    try {
      const typeIds = [...new Set([shipId, ...moduleTypeIds])].filter((id) => id > 0)

      // GET (not POST — /api/market/prices only exposes GET) with the real module
      // typeIds, so the fit total reflects actual Jita prices instead of a flat
      // 1M ISK/module placeholder.
      const res = await fetch(`/api/market/prices?typeIds=${typeIds.join(',')}`)

      if (res.ok) {
        const prices: Record<string, number> = await res.json()
        const shipPrice = prices[shipId] || 0
        const modulesTotal = moduleTypeIds.reduce((sum, id) => sum + (prices[id] || 0), 0)

        setPricing({
          ship: shipPrice,
          fit: shipPrice + modulesTotal
        })
      }
    } catch (err) {
  ...
  }, [stats, shipId, moduleCount, moduleTypeIds])
