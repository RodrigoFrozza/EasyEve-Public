      ...
      <CardContent className="relative space-y-3 p-4">
        ...
        <div className="flex flex-wrap items-center gap-x-1.5 gap-y-1 text-[11px] text-muted-foreground">
          <span className="flex items-center gap-1">
            ...
            <span className="font-medium text-foreground">{fit.modules.length}</span> mod
          </span>
          {fit.drones.length > 0 && (
            <>
              <span className="text-border">·</span>
              <span><span className="font-medium text-foreground">{fit.drones.length}</span> drones</span>
            </>
          )}
          {fit.cargo.length > 0 && (
            <>
              <span className="text-border">·</span>
              <span><span className="font-medium text-foreground">{fit.cargo.length}</span> cargo</span>
            </>
          )}
          <span className="text-border">·</span>
          <span className="inline-flex items-center gap-1">
            <Clock className="h-3.5 w-3.5" />
            {formatDistanceToNow(new Date(fit.updatedAt), { addSuffix: true })}
          </span>
        </div>
      </CardContent>
