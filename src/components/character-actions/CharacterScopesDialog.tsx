...
export function CharacterScopesDialog({
  scopes,
  esiApp = 'main',
  open: controlledOpen,
  onOpenChange: controlledOnOpenChange,
  hideTrigger = false,
}: CharacterScopesDialogProps) {
      ...
      {!hideTrigger && (
        <DialogTrigger asChild>
          <Button
            ...
            className={cn(
              'border-eve-border',
              hasMissing && 'border-red-500/50 text-red-400 hover:bg-red-500/10'
            )}
          >
            <List className="mr-2 h-4 w-4" />
            Scopes
            {hasMissing && (
              <span className="ml-1.5 flex h-4 w-4 items-center justify-center rounded-full bg-red-500 text-[8px] font-bold text-white">
                !
              </span>
            )}
          </Button>
