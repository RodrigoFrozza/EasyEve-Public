...
export function AutoRefreshManager({ 
  ...
  useEffect(() => {
    ...
    const needsDataSync = (now - lastSync > SIX_HOURS)
    ...
    if (needsDataSync || expiredTokenChar) {
      ...
      if (needsDataSync) {
        toast.info(`Automated Fleet Sync: Refreshing character data...`, {
          description: `Last sync was ${timeAgo(new Date(lastSync))}`
        })
      }
      
      fetch('/api/characters/refresh-all', { method: 'POST' })
        .then(res => {
          if (res.ok) {
