...
export function CharactersFilters({ characters, serverDriven = false, children }: CharactersFiltersProps) {
  ...
  const updateURL = useCallback(
    (search: string, sort: SortField, order: SortOrder, status: StatusFilter, tag: string) => {
      ...
      if (status === 'tag' && tag) params.set('tag', tag)
  ...
  const filteredCharacters = useMemo(() => {
    if (serverDriven) return characters // skip expensive client-side filtering when server-driven
  ...
  return (
    <div className="space-y-4">
      ...
      {serverDriven && (
        <p className="text-xs text-zinc-500">{t('characters.filters.serverFilteringHint')}</p>
      )}
