        ...
        <div className="flex flex-wrap gap-2">
          {activeActivities.map((activity, index) => (
            <Link 
              ...
              className="flex items-center gap-3 px-4 py-2 bg-emerald-500/5 border border-emerald-500/10 rounded-full hover:bg-emerald-500/10 transition-all cursor-pointer"
            >
              <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
              <span className="text-xs font-medium text-zinc-300 uppercase">{activity.type}</span>
              <span className="text-[10px] text-zinc-500">
                <FormattedDate date={activity.startTime} mode="time" />
              </span>
            </Link>
          ))}
        </div>
