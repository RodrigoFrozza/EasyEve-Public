...
function getActivityEarnings(activity: ActivityData): number {
  ...
  if (type === 'MINING') {
    return Number(data.miningValue || data.totalEstimatedValue || 0)
  }
  if (type === 'RATTING') {
    return Number(data.automatedBounties || 0) 
  ...
  if (type === 'EXPLORATION') {
    return Number(data.totalLootValue || data.totalEstimatedValue || 0)
  }
  return 0
}
        ...
        {activities.length === 0 ? (
          ...
          activities.slice(0, 5).map((activity) => {
            ...
            const duration = activity.endTime
              ...
              : differenceInMinutes(new Date(), activity.startTime)
