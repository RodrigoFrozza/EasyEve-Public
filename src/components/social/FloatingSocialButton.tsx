...
export function FloatingSocialButton() {
  ...
  const { data: globalSearchData, isLoading: searchingGlobal } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await axios.get(`/api/players/search?q=${encodeURIComponent(debouncedSearch)}`)
      return res.data
    },
  ...
  const displayList = useMemo(() => {
    let list = []
    if (activeTab === 'friends') list = contacts
    else if (activeTab === 'pending') list = pendingReceived
    else if (activeTab === 'sent') list = pendingSent
    
    if (!searchQuery) return list
    return list.filter((item: any) => 
      item.name.toLowerCase().includes(searchQuery.toLowerCase())
    )
  }, [activeTab, contacts, pendingReceived, pendingSent, searchQuery])
  ...
  const handleSendRequest = async (userId: string) => {
    try {
      await axios.post(`/api/players/${userId}/contact`)
      queryClient.invalidateQueries({ queryKey: ['contacts'] })
      setSearchQuery('')
      setActiveTab('sent')
    } catch (err) {
  ...
  const onlineFriends = displayList.filter((c: Contact) => c.isOnline)
  const offlineFriends = displayList.filter((c: Contact) => !c.isOnline)
                            ...
                            <div className="space-y-1">
                                ...
                                <div className="space-y-1">
                                  {onlineFriends.map((contact: Contact) => (
                                    <ContactItem 
                                      ...
                                      t={t} 
                                      router={router} 
                                      onRemove={() => handleCancelRequest(contact.id)}
                                    />
                                  ))}
                                </div>
                              )}
                              ...
                              {contacts.length === 0 && !searchQuery && (
                                <div className="py-20 text-center">
                                  ...
                                  <Button 
                                    variant="ghost" 
                                    className="mt-4 text-eve-accent text-[10px] font-bold uppercase tracking-widest"
                                    onClick={() => {
                              ...
                              {/* Global Search Results in Friends Tab when searching */}
                              {searchQuery.length >= 2 && (
                                <div className="mt-6 border-t border-white/5 pt-4">
                                  <div className="px-3 py-2 flex items-center gap-2">
                                    <Globe className="h-3 w-3 text-eve-accent/60" />
                                    <span className="text-[9px] font-black text-gray-500 uppercase tracking-[0.2em]">Registry Search</span>
                                    {searchingGlobal && <Loader2 className="h-2 w-2 animate-spin text-eve-accent" />}
                                  </div>
                                  
                                  <div className="space-y-1">
                                    {globalSearchData?.users?.filter((u: any) => 
                                      !contacts.some((c: any) => c.id === u.id) &&
                                      !pendingSent.some((s: any) => s.id === u.id) &&
                                      !pendingReceived.some((r: any) => r.id === u.id) &&
                                      u.id !== (session?.user as any)?.id
                                    ).map((user: any) => (
                                      <div key={user.id} className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-white/[0.04] transition-all border border-transparent group">
                                        <Avatar className="h-10 w-10 border border-white/5">
                                          <AvatarImage src={`https://images.evetech.net/characters/${user.mainCharacterId}/portrait?size=64`} />
                                          <AvatarFallback>?</AvatarFallback>
                                        </Avatar>
                                        <div className="flex-1 min-w-0">
                                          <div className="text-xs text-white font-bold truncate">{user.name}</div>
                                          <div className="text-[9px] text-gray-500 truncate">{user.corporation || 'No Corporation'}</div>
                                        </div>
                                        <Button
                                          size="sm"
                                          className="h-8 bg-eve-accent/10 hover:bg-eve-accent text-eve-accent hover:text-black border border-eve-accent/20 text-[9px] font-bold uppercase tracking-tighter"
                                    ...
                                    {globalSearchData?.users?.length === 0 && !searchingGlobal && (
                                      <p className="px-3 py-4 text-[10px] text-gray-600 italic">No pilots found in the registry.</p>
                                    )}
                                  </div>
                                </div>
                              )}
                              ...
                              {displayList.length > 0 ? (
                                ...
                                <div className="py-20 text-center opacity-40">
                                  ...
                                  <p className="text-[10px] font-bold uppercase tracking-widest text-gray-500">
                                    {searchQuery ? 'No matching signals' : 'Zero Pending Signals'}
                                  </p>
                                </div>
                              )}
                            </div>
                            ...
                            <div className="space-y-2 py-1">
                              {displayList.length > 0 ? (
                                ...
                                <div className="py-20 text-center opacity-40">
                                  <Filter className="h-12 w-12 text-gray-600 mx-auto mb-4" />
                                  <p className="text-[10px] font-bold uppercase tracking-widest text-gray-500">
                                    {searchQuery ? 'No matching transmissions' : 'No Active Transmissions'}
                                  </p>
                                </div>
                              )}
                            </div>
...
function ContactItem({ contact, t, router, onRemove }: { contact: Contact, t: any, router: any, onRemove?: () => void }) {
    ...
    <div className="relative">
      <motion.div
        ...
        className={cn(
          ...
          showRemoveConfirm && "bg-rose-500/10 border-rose-500/20"
        )}
        onClick={() => !showRemoveConfirm && router.push(`/players/${contact.id}`)}
      >
        <div className="relative">
        ...
        <div className="flex-1 min-w-0">
          <div className={cn(
            "text-xs truncate font-bold tracking-tight flex items-center gap-1.5",
            contact.isOnline ? "text-white" : "text-gray-400"
          )}>
            {contact.name}
            {contact.isOnline && contact.isTester && (
              <Badge className="bg-blue-500/20 text-blue-400 border-blue-500/50 text-[8px] px-1 py-0 font-bold uppercase">
                Tester
              </Badge>
            )}
          </div>
          ...
          <div className="flex items-center gap-1">
            {showRemoveConfirm ? (
              <div className="flex items-center gap-1 animate-in fade-in slide-in-from-right-2">
                <Button
                  size="icon"
                  className="h-7 w-7 rounded-lg bg-rose-600 hover:bg-rose-700 text-white border-none"
                  onClick={(e) => {
                    e.stopPropagation();
                    onRemove();
              ...
              <Button
                size="icon"
                variant="ghost"
                className="h-8 w-8 rounded-lg text-gray-500 hover:text-rose-500 hover:bg-rose-500/10 opacity-0 group-hover:opacity-100 transition-all"
                onClick={(e) => {
                  e.stopPropagation();
