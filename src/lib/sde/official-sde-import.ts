...
export function assertUnzipAvailable(): void {
  try {
 ...
 */
export function findZipEntryName(zipPath: string, entryBasename: string): string {
...
export async function streamLinesFromZip(
  zipPath: string,
  innerPath: string
    ...
    const marketGroupId =
      typeof o.marketGroupID === 'number'
        ...
        : o.marketGroupID != null && !Number.isNaN(Number(o.marketGroupID))
          ...
          : null
    const metaRaw = (o as { metaGroupID?: unknown }).metaGroupID
    const metaGroupId =
      typeof metaRaw === 'number'
        ? Math.floor(metaRaw)
        : metaRaw != null && !Number.isNaN(Number(metaRaw))
          ? Math.floor(Number(metaRaw))
          : null
