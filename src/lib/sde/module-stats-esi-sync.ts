  ...
  await batchPromiseAll(
    ...
    async (typeId, idx) => {
      ...
      try {
        ...
        // Root cause of optimalRange/fireRate/falloffRange/trackingSpeed always reading 0: this used
        // to gate on MODULE_GROUPS.{ENERGY,PROJECTILE,HYBRID}_TURRET / MISSILE_LAUNCHER (76-79), which
        // are NOT real EVE group ids for those categories — real turret/launcher groups are per-size
        // (e.g. Light Neutron Blaster II's real group is 74; Caldari Navy Cruise Missile Launcher's is
        // 506). Those 4 numbers never matched a real module, so isWeapon/isMissile were always false
        // and every dependent field below was force-zeroed regardless of whether its attribute id was
        // correct. Use the dogma-effect-based flags instead — reliable across all sizes/races.
        const isWeapon = isTurret || isLauncher
        const isMissile = isLauncher
        ...
        const stats = {
          ...
          // attr 50/30, confirmed via LNB II. Not gated — cpu/powerGrid are naturally sparse (0 on
          // modules that don't need fitting resources).
          cpu: attrs[DOGMA_ATTRS.CPU_NEEDED] || 0,
          powerGrid: attrs[DOGMA_ATTRS.POWER_NEEDED] || 0,

          // damage: no valid module-level attribute (ammo-dependent, see DAMAGE comment above) — always 0.
          ...
          // missileDamage/Range/Velocity: no valid launcher-level attribute (ammo-dependent, see
          // MISSILE_* comments above) — always 0. isMissile itself is now correctly derived from the
          // LAUNCHER_FITTED effect (see isWeapon/isMissile comment above).
          ...
          // MODULE_GROUPS.SHIELD_BOOSTER/ARMOR_REPAIR/HULL_REPAIR (29/30/31) are not real EVE module
          // groups — they are the real group ids for the Capsule/Titan/Shuttle SHIP categories (never
          // present among synced module types), so these gates were always false. Real group ids
          // confirmed via ESI: Shield Booster = 40, Armor Repair Unit = 62 (Medium Armor Repairer II,
          // typeId 3530). Hull Repair Unit's real group id is not yet validated — kept at 0 (see
          // HULL_BOOST comment in dogma-attribute-ids.ts).
          shieldBoost: groupId === 40 ? attrs[DOGMA_ATTRS.SHIELD_BOOST] || 0 : 0,
          armorRepair: groupId === 62 ? attrs[DOGMA_ATTRS.ARMOR_BOOST] || 0 : 0,
          hullRepair: 0,

          capacitorNeed: attrs[DOGMA_ATTRS.CAPACITOR_NEEDED] || 0,
          capacitorDrain: 0,

          // MODULE_GROUPS.ECM/SENSOR_DAMP/TRACKING_DISRUPTOR (111/112/113) are likewise not real module
          // groups (111 is "Shuttle Blueprint"), so these were always 0 regardless of attribute id.
          // Real group ids + correct attributes are not yet validated against a reference module — see
          // dogma-attribute-ids.ts comments for ECCM_SENSOR_STRENGTH / SENSOR_DAMPENER_RANGE /
          // TRACKING_DISRUPTOR_RANGE. Kept at 0 rather than guessing.
          ecmStrength: 0,
          sensorDampStrength: 0,
          trackingDisruptStrength: 0,
          // Stasis Web (group 65) IS a real, confirmed group id — this gate already worked. The
          // attribute ids were wrong: confirmed via Stasis Webifier I (typeId 526) that speed factor is
          // attr 20 and range is attr 54 (the same universal `maxRange` attribute turrets use).
          ...
          // shieldBonus: attr 72 (capacityBonus), confirmed via Large Shield Extender II. Not gated by
          // groupId — the attribute is naturally sparse (only present on shield-HP modules), so no
          // false positives once the id is correct.
          shieldBonus: attrs[DOGMA_ATTRS.SHIELD_CAPACITY] || 0,
          armorBonus: attrs[DOGMA_ATTRS.ARMOR_HP] || 0,
          // hullBonus: no validated flat hull-HP-bonus attribute id (see STRUCTURAL_INTEGRITY comment
          // in dogma-attribute-ids.ts) — sentinel keeps this at 0 instead of the old junk value (~40).
