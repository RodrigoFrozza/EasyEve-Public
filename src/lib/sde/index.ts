...
// The EVE image server (images.evetech.net) expects a numeric `size` query
// param (one of 32/64/128/256/512/1024) — a word like `size=large` is not a
// valid value and yields no image.
export type EveImageSize = 32 | 64 | 128 | 256 | 512 | 1024

export function getCharacterPortraitUrl(characterId: number, size: EveImageSize = 128): string {
...
export function getCorporationLogoUrl(corpId: number, size: EveImageSize = 128): string {
  return `${EVE_IMAGES_URL}/corporations/${corpId}/logo?size=${size}`
}

export function getAllianceLogoUrl(allianceId: number, size: EveImageSize = 128): string {
...
const constellationRegionCache = new Map<number, number>()
...
/**
 * ESI's /universe/systems/{id}/ response has no region_id field — only
 * constellation_id. Region comes from a second hop through
 * /universe/constellations/{id}/, cached separately since many systems
 * share the same constellation.
 */
async function getRegionIdForConstellation(constellationId: number): Promise<number | undefined> {
  ...
  try {
    ...
    const regionId = response.data?.region_id as number | undefined
    if (regionId) {
      setWithLimit(constellationRegionCache, constellationId, regionId)
    }
    return regionId
  } catch (error) {
    logger.error('SDE', `Error fetching constellation info for ${constellationId}`, error)
    return undefined
  }
}
...
export async function getSolarSystemInfo(systemId: number): Promise<SystemInfo | null> {
  ...
  try {
    const response = await esiClient.get(`/universe/systems/${systemId}/`)
    const data = response.data
    const regionId = await getRegionIdForConstellation(data.constellation_id)
...
export interface SkillTypeCatalogEntry {
  id: number
  name: string
  groupId: number
  groupName: string
...
/**
 * Batch resolve skill type IDs to their name + SDE group (for grouping a
 * character's trained skills / skill queue in the UI). Local-DB only — like
 * `getGroupInfo` above, the SDE `EveType`/`EveGroup` tables are expected to be
 * fully synced, so no ESI fallback here. Any ID that isn't found locally is
 * simply omitted from the result (never fabricated) — callers must degrade
 * gracefully (e.g. show "Unknown skill (ID ####)") instead of guessing a name.
 */
export async function resolveSkillCatalog(typeIds: number[]): Promise<Record<number, SkillTypeCatalogEntry>> {
  const uniqueIds = Array.from(new Set(typeIds))
  const result: Record<number, SkillTypeCatalogEntry> = {}
  ...
  for (const row of rows) {
    result[row.id] = { id: row.id, name: row.name, groupId: row.groupId, groupName: row.group.name }
  }
