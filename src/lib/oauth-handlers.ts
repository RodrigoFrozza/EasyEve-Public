...
export async function handleLoginFlow(
  ...
  if (existingChar) {
    // Never downgrade a designated Holding character to 'main' via a normal
    // login. Doing so silently strips its corp-wallet scopes and breaks the
    // payment sync (it was 26 days of silent failures that motivated this).
    // A demotion must be an explicit admin action, not a stray re-login.
    const nextEsiApp = existingChar.esiApp === 'holding' && esiApp !== 'holding'
      ? 'holding'
      : esiApp
    if (nextEsiApp !== esiApp) {
      logger.warn('OAuthLogin', `Preserving 'holding' designation for ${characterName} (${characterId}) despite ${esiApp} login`, { characterId })
    }
    ...
    await prisma.character.create({
      data: {
        ...
        isMain: true,
        esiApp,
        birthday: charData.birthday ? new Date(charData.birthday) : undefined,
        raceId: charData.raceId,
        bloodlineId: charData.bloodlineId,
        gender: charData.gender,
        securityStatus: charData.securityStatus,
      },
    })
    userId = user.id
  }
...
export async function handleLinkFlow(
  ...
  if (existingChar) {
    // Never downgrade a Holding character to 'main' via a normal link (see the
    // matching guard in handleLoginFlow) — that silently strips corp-wallet
    // scopes and breaks the payment sync.
    const nextEsiApp = existingChar.esiApp === 'holding' && esiApp !== 'holding'
      ? 'holding'
      : esiApp
    if (nextEsiApp !== esiApp) {
      logger.warn('OAuthLink', `Preserving 'holding' designation for ${characterName} (${characterId}) despite ${esiApp} link`, { characterId })
    }
    await prisma.character.update({
      where: { id: characterId },
      data: {
        ...
        tokenExpiresAt: new Date(Date.now() + tokenData.expires_in * 1000),
        name: characterName,
        esiApp: nextEsiApp, // Ensure the app context is updated (never downgraded)
      },
    })
  } else {
    const charData = await fetchCharacterData(characterId, tokenData.access_token)
