...
export const ACTIVITY_TYPES: ActivityTypeOption[] = [
  ...
  { id: 'crab', label: 'Crab Beacon', comingSoon: true },
  { id: 'escalations', label: 'Escalations' },
  { id: 'pvp', label: 'PVP', comingSoon: true },
]
...
/** Wreck loot factions selectable when launching a salvaging session */
export const SALVAGE_NPC_FACTIONS = NPC_FACTIONS

export function getRattingNpcFaction(activity: {
  data?: Record<string, unknown> | null
}): string | undefined {
  const data = activity.data as { npcFaction?: string } | undefined
  const faction = data?.npcFaction?.trim()
  return faction || undefined
}

export function getSalvagingNpcFaction(activity: {
  region?: string | null
  data?: Record<string, unknown> | null
}): string | undefined {
  const data = activity.data as { npcFaction?: string; region?: string } | undefined
  return (
    data?.npcFaction?.trim() ||
    data?.region?.trim() ||
    activity.region?.trim() ||
    undefined
  )
}
