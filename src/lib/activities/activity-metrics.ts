...
export function getActivityFinancialMetrics(activity: { type?: string; data?: AnyRecord }): ActivityFinancialMetrics {
  ...
  const runs = (data as AnyRecord)?.runs as Array<{ lootValue?: number }> | undefined
  const logs = (data as AnyRecord)?.logs as Array<{ amount?: number }> | undefined
  
  const totalLootValue = toNumber((data as AnyRecord)?.totalLootValue)
    || (runs?.reduce((sum: number, run) => sum + toNumber(run.lootValue), 0) || 0)
    || (logs?.reduce((sum: number, log) => sum + toNumber(log.amount), 0) || 0)
