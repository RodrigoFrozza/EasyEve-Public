...
export const launchActivityTypeSchema = z.enum([
  'mining',
  'ratting',
  'abyssal',
  'exploration',
...
export const explorationActivityDataSchema = z.object({
  lastCargoState: z.string().max(20000).optional(),
  siteType: z.string().max(120).optional(),
  region: z.string().max(180).optional(),
  ...autoLootFieldsSchema,
})

export const salvagingActivityDataSchema = z.object({
  lastCargoState: z.string().max(20000).optional(),
  npcFaction: nonEmptyString(120).optional(),
  /** @deprecated Legacy sessions stored empire region here */
  region: z.string().max(180).optional(),
  ...autoLootFieldsSchema,
})

export const escalationsActivityDataSchema = z.object({
  lastCargoState: z.string().max(20000).optional(),
  npcFaction: z.string().max(120).optional(),
})
...
export function validateLaunchActivityInput(input: LaunchInput): ActivityLaunchValidationResult {
  ...
  if (!typeResult.success) {
    issues.push(
      'Activity type must be mining, ratting, abyssal, exploration, salvaging, or escalations'
    )
  }
  ...
  if (type === 'exploration') {
    ...
    if (!explorationResult.success) {
      issues.push('Invalid configuration for exploration activities')
    } else {
      checkAutoLootRules(explorationResult.data, issues)
    }
  }

  if (type === 'salvaging') {
    const npcFaction = (typeof data.npcFaction === 'string' ? data.npcFaction : '').trim()
    if (!npcFaction) {
      issues.push('NPC wreck faction is required for salvaging')
    }
    const salvagingResult = salvagingActivityDataSchema.safeParse({
      ...data,
      npcFaction: npcFaction || undefined,
    })
    if (!salvagingResult.success) {
      issues.push('Invalid configuration for salvaging activities')
    } else {
      checkAutoLootRules(salvagingResult.data, issues)
    }
  }

  if (type === 'escalations') {
    const escalationsResult = escalationsActivityDataSchema.safeParse(data)
    if (!escalationsResult.success) {
      issues.push('Invalid configuration for escalations activities')
    }
  }
