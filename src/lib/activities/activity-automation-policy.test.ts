...
describe('activity-automation-policy', () => {
  describe('getLastActivityInstantForAudit', () => {
    it('uses the latest of lastSyncAt, lastDataAt, and updatedAt', () => {
      const sync = new Date('2024-01-01T12:00:00Z')
      const rowUpdate = new Date('2024-01-02T00:00:00Z')
      const instant = getLastActivityInstantForAudit({
        data: { lastSyncAt: sync.toISOString() },
        updatedAt: rowUpdate,
      })
      expect(instant.toISOString()).toBe(rowUpdate.toISOString())
    })

    it('prefers lastSyncAt when it is newer than updatedAt', () => {
  ...
  describe('isStaleForAutoPause', () => {
    it('returns false when within 60 minutes of last activity', () => {
    ...
    it('returns true when more than 60 minutes since last activity', () => {
      const now = Date.parse('2024-01-01T14:00:00Z')
      const last = new Date('2024-01-01T12:00:00Z')
      expect(
        isStaleForAutoPause({ data: { lastSyncAt: last.toISOString() }, updatedAt: last }, now)
      ).toBe(true)
    })
