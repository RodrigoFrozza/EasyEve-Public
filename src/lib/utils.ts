...
export function hasPremiumAccess(input: {
  ...
  return Boolean(input.isTester) || isPremium(input.subscriptionEnd)
}
...
/** Premium that does not require monthly ISK renewal (lifetime / far-future end dates). */
const LIFETIME_PREMIUM_MIN_REMAINING_DAYS = 3650

export function isLifetimePremium(
  subscriptionEnd: string | Date | null | undefined
): boolean {
  return getRemainingDays(subscriptionEnd) >= LIFETIME_PREMIUM_MIN_REMAINING_DAYS
}
...
export function calculateNetProfit(data: any): number {
  if (!data) return 0
  const bounties =
    (data.automatedBounties || 0) +
    (data.automatedEss || 0) +
    (data.additionalBounties || 0)
  const assets =
    (data.estimatedLootValue || 0) +
    (data.estimatedSalvageValue || 0) +
    (data.estimatedEscalationValue || 0)
  const taxes = data.automatedTaxes || 0
  const expenses = data.estimatedExpenseValue || 0
  return Math.max(0, bounties - taxes) + assets - expenses
}
