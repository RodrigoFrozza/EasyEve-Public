...
import { extractAccountCode } from '@/lib/account-code'
...
export async function syncCorporationPayments() {
  ...
  for (const entry of journal) {
    ...
    // Resolve user. The payer's typed note (with the account code) lives in
    // `reason`; `description` is CCP's auto-generated text — scan both.
    let resolvedUserId: string | null = null
    const accountCode = extractAccountCode(entry.reason, entry.description)
    ...
    // Create payment + credit ISK atomically — a mid-way crash must never leave
    // a payment without its balance credit (or vice versa). History is best-effort.
...
export async function autoRefreshToken() {
  ...
  for (const char of characters) {
    ...
    try {
      ...
      const refreshResult = await refreshAccessToken(decryptToken(char.refreshToken), char.esiApp || 'main')
