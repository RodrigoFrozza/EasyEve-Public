  ...
  try {
    ...
    await prisma.scriptExecution.update({
      ...
      data: {
        ...
        // Persist the message to the dedicated column too — previously it only
        // landed in `logs`, leaving `error` null and failures invisible in the
        // admin UI (a corp-wallet sync failed silently for 26 days).
        error: msg,
      },
    })
  }
