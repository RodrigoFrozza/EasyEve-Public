...
import { encryptToken, safeDecryptToken } from './crypto/token-cipher'
...
export async function getValidAccessToken(characterId: number): Promise<{ accessToken: string | null; characterId: number; error?: string }> {
  ...
  const performRefresh = async () => {
    try {
      ...
      const storedAccessToken = safeDecryptToken(character.accessToken)
      const storedRefreshToken = safeDecryptToken(character.refreshToken)

      // A refresh token that won't decrypt (key rotation / corruption) means we
      // can't recover this character's session — fail cleanly instead of throwing.
      if (!storedRefreshToken) {
        logger.error('TokenManager', `Failed to decrypt refresh token for character ${characterId}`)
        return { accessToken: null, characterId, error: 'decrypt_failed' }
      }
      ...
      if (storedAccessToken && expiresAt && expiresAt.getTime() > now.getTime() + 5 * 60 * 1000) {
        return { accessToken: storedAccessToken, characterId }
      }

      logger.info('TokenManager', `Refreshing token for character ${characterId}`, { esiApp: character.esiApp })
      const refreshResult = await refreshAccessToken(storedRefreshToken, character.esiApp)
