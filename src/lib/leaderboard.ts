...
export async function getLeaderboardData(
  ...
  const fetchData = async (): Promise<LeaderboardItem[]> => {
    ...
    if (normalizedType === 'RATTING') {
      aggregationSql = prisma.$queryRaw`
        SELECT 
          a."userId",
          SUM(
        ...
        GROUP BY a."userId"
        ORDER BY total DESC
        LIMIT ${limit * 2} -- Fetch more to account for users without main characters if any
      `
    ...
    const userIds = aggregatedResults.map(r => r.userId)
    ...
    return aggregatedResults
      .map(r => {
        const u = userDetailsMap.get(r.userId)
        const mainChar = u?.characters[0]
        return {
          userId: r.userId,
      ...
      .filter(item => item.characterId !== 0) // Only show users with active main characters
      .slice(0, limit)
  }
