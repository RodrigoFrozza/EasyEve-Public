...
export function aggregateActivityData(
  ...
  if (type === 'RATTING') {
    ...
    const escalations = Number(data.estimatedEscalationValue || 0)
    result.label1 = bounty
    result.label2 = ess
    result.total = bounty + ess + additional + loot + salvage + escalations
  } else if (type === 'MINING') {
      ...
      aggregationSql = prisma.$queryRaw<Array<{
        ...
        SELECT 
          ...
          SUM(
            ...
            COALESCE((a.data->>'estimatedEscalationValue')::numeric, 0)
          ) as total,
