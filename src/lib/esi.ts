...
export async function getCharacterAssetNames(characterId: number, itemIds: number[]) {
  try {
    ...
    const response = await esiClient.post(`/characters/${characterId}/assets/names/`, itemIds, {
      headers: { Authorization: `Bearer ${accessToken}` },
    })
    return response.data as Array<{ item_id: number; name: string }>
  } catch (error) {
...
export async function getStructureName(structureId: number, characterId: number) {
  try {
    ...
    logger.error('ESI', `Failed to fetch structure name for ${structureId}`, error)
