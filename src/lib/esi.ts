...
import { fetchMiningLedgerWithToken } from '@/lib/esi/mining-ledger'
...
export async function getCharacterSkills(characterId: number, accessToken: string): Promise<CharacterSkills> {
  ...
  return {
    total_sp: skills.total_sp,
    // ESI's field is `unallocated_sp`, and it is OMITTED from the payload when it
    // is 0 (not sent as 0). Since we only reach here on a successful skills fetch,
    // an absent value means the character genuinely has 0 unallocated SP — coalesce
    // to 0 rather than leaving it undefined (which persisted as null → "No data").
    free_sp: skills.unallocated_sp ?? 0,
    skills: skills.skills || [],
    queues: queue || [],
  }
}

// Scope: esi-skills.read_skills.v1 (already requested — see src/lib/constants/scopes.ts)
export async function getCharacterAttributes(characterId: number, accessToken: string): Promise<CharacterAttributes> {
  const cacheKey = `char-attributes-${characterId}`

  return withCache(cacheKey, async () => {
    const response = await esiClient.get(`/characters/${characterId}/attributes/`, {
      headers: { Authorization: `Bearer ${accessToken}` },
    })
    return CharacterAttributesSchema.parse(response.data)
  }, 600000) // 10 minutes cache
}

// Scope: esi-clones.read_implants.v1 (see src/lib/constants/scopes.ts)
export async function getCharacterImplants(characterId: number, accessToken: string): Promise<number[]> {
  const cacheKey = `char-implants-${characterId}`

  return withCache(cacheKey, async () => {
    const response = await esiClient.get(`/characters/${characterId}/implants/`, {
      headers: { Authorization: `Bearer ${accessToken}` },
    })
...
export async function getCharacterMiningLedger(
  ...
  try {
    return await fetchMiningLedgerWithToken(characterId, accessToken)
  } catch (error) {
...
export async function getMarketPrices(): Promise<Record<number, number>> {
  ...
  return withCache(cacheKey, async () => {
    try {
      ...
      // Must not swallow this into `{}` — withCache would persist that empty map as a
      // "successful" 5-minute cache entry, making every price lookup return nothing for
      // everyone until it expires, even though the ESI outage may have been momentary.
      throw error
    }
...
export async function getStructureNamesBatch(structureIds: number[], characterId: number): Promise<Map<number, string>> {
  ...
  try {
    ...
    for (const item of namesData) {
      if (item.category === 'structure' || item.category === 'station') {
...
export type TrackableLocationCategory = 'station' | 'structure'

export type TrackableLocation = {
  name: string
  category: TrackableLocationCategory
}

const UPWELL_STRUCTURE_MIN_ID = 1_000_000_000
const MAX_INDIVIDUAL_STRUCTURE_LOOKUPS = 20

/** NPC station range, or the (loose) Upwell structure range — anything else (e.g. an
 *  item_id of a fitted module or cargo sitting inside a ship/container) can never
 *  resolve to a station/structure and would make the ESI batch call reject the
 *  whole request if included. */
function isPlausibleTrackableLocationId(id: number): boolean {
  if (id >= 60_000_000 && id < 64_000_000) return true
  if (id >= UPWELL_STRUCTURE_MIN_ID) return true
  return false
}

export async function resolveTrackableLocationCategories(
  locationIds: number[],
  characterId?: number
): Promise<Map<number, TrackableLocation>> {
  const result = new Map<number, TrackableLocation>()
  const uniqueIds = [...new Set(locationIds)].slice(0, 1000)

  if (uniqueIds.length === 0) {
    return result
  }

  const plausibleIds = uniqueIds.filter(isPlausibleTrackableLocationId)

  if (plausibleIds.length > 0) {
    try {
      const response = await esiClient.post('/universe/names/', plausibleIds)
      const namesData = response.data as Array<{ id: number; name: string; category: string }>

      for (const item of namesData) {
        if (item.category === 'station' || item.category === 'structure') {
          result.set(item.id, {
            name: item.name,
      ...
      logger.error('ESI', 'Failed to resolve trackable location categories in batch', error)
    }
  }

  const missingIds = plausibleIds.filter((id) => !result.has(id))

  for (const id of missingIds) {
    if (id >= 60_000_000 && id < 64_000_000) {
      const name = await getStationName(id)
      if (name !== 'Unknown Station') {
        result.set(id, { name, category: 'station' })
      }
    }
  }

  if (characterId) {
    const structureCandidates = missingIds
      .filter((id) => id >= UPWELL_STRUCTURE_MIN_ID && !result.has(id))
      .slice(0, MAX_INDIVIDUAL_STRUCTURE_LOOKUPS)

    for (const id of structureCandidates) {
      const cached = structureNamesCache.get(id)
      if (cached && Date.now() - cached.timestamp < STRUCTURE_CACHE_TTL) {
        result.set(id, { name: cached.name, category: 'structure' })
        continue
      }

      const info = await getStructureInfo(id, characterId)
      if (info) {
        result.set(id, { name: info.name, category: 'structure' })
        structureNamesCache.set(id, { name: info.name, timestamp: Date.now() })
      }
    }
  }
...
export async function resolveLocationName(locationId: number, characterId: number) {
  if (locationId >= 60000000 && locationId < 64000000) {
    ...
    // A deployable (e.g. Mobile Tractor Unit) sitting in space reports the
    // solar system as its location, not a station/structure.
    try {
      return await getSolarSystemName(locationId)
    } catch {
...
async function fetchPaginatedJournal(
  ...
  // Journal is newest-first; once the oldest entry on a page predates untilDate
  // we've fetched everything we need and can stop early.
  const reachedUntil = (data: WalletJournal[]): boolean =>
    untilDate ? new Date(data[data.length - 1].date) < untilDate : false

  const firstResponse = await esiClient.get(url, {
    params: { ...params, page: 1 },
    headers: { Authorization: `Bearer ${accessToken}` },
  })
  const firstData = firstResponse.data
  if (!Array.isArray(firstData) || firstData.length === 0) return results
  results.push(...(firstData as WalletJournal[]))

  // ESI reports the real total page count in the `x-pages` header (pages hold up
  // to 2500 entries) — trust it instead of guessing from the array length, which
  // could stop early on a partial page or loop needlessly. Cap at 50 as a guard.
  const totalPages = Math.min(parseInt(firstResponse.headers['x-pages'] || '1', 10) || 1, 50)
  if (reachedUntil(firstData as WalletJournal[])) return results
  ...
  for (let page = 2; page <= totalPages; page++) {
    ...
    if (reachedUntil(data as WalletJournal[])) break
  }
