...
/**
 * Rejects a fit that would be persisted despite exceeding CPU/powergrid/slot limits
 * or referencing modules/charges unknown to the module catalog. Skips validation for
 * a draft fit with no hull selected yet (shipTypeId falsy) — there is nothing to
 * validate against.
 */
export async function assertFittingIsValid(params: {
  shipTypeId?: number | null
  modules: unknown[]
}): Promise<void> {
  if (!params.shipTypeId) return

  const result = await validateFittingState({
    shipTypeId: params.shipTypeId,
    modules: normalizeEditorModules(params.modules),
  })

  if (!result.success) {
    throw new AppError(
      ErrorCodes.VALIDATION_FAILED,
      `Fit is not valid: ${result.errors.join('; ') || 'exceeds ship CPU/powergrid/slot limits'}`,
      400
    )
  }
}
