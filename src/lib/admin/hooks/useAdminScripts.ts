...
export function useEnsureRequiredSchedules() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: async () => {
      const res = await fetch('/api/admin/scripts/schedules/ensure-required', {
        method: 'POST',
      })
      if (!res.ok) throw new Error('Failed to create recommended schedules')
