...
export interface MarketAppraisalDetail {
  ...
  buyPrice: number
  sellPrice: number
  source: 'jita_buy' | 'jita_sell' | 'global_average' | 'not_found'
  liquidity: number
}
...
async function fetchMarketDataInternal(itemNames: string[]): Promise<Record<string, { price: number, id: number }>> {
  ...
  // Fallback price list is the same ~14k-item payload for every type_id — fetch it at
  // most once per call (lazily, only if actually needed) instead of once per item, or
  // a batch of low-liquidity items would trigger that many concurrent full downloads.
  let globalPricesPromise: Promise<MarketPrice[]> | null = null
  const getGlobalPrices = (): Promise<MarketPrice[]> => {
    if (!globalPricesPromise) {
      globalPricesPromise = esiClient
        .get(`/markets/prices/`)
        .then((res) => res.data as MarketPrice[])
        .catch((err) => {
          logger.error('Market', 'Global price list fetch failed', { error: err })
          return []
        })
    }
    return globalPricesPromise
  }
  ...
  await batchPromiseAll(
    ...
    async ([name, info]) => {
      try {
        ...
        let source: MarketAppraisalDetail['source'] = buyPrice > 0 ? 'jita_buy' : sellPrice > 0 ? 'jita_sell' : 'not_found'
        let unitPrice = source === 'jita_buy' ? buyPrice : source === 'jita_sell' ? sellPrice : 0

        if (unitPrice === 0 && info.price > 0) {
          // No Jita 4-4 order book depth for this item — fall back to the same
          // global-average price getMarketAppraisal/WithIds already use (with
          // their own 6h cache), so the same item never disagrees in value
          // depending on which registration flow (Loot/MTU/Salvage) priced it.
        ...
        const fallbackPrice = info.price || 0
        result[name] = {
          typeId: info.id,
          unitPrice: fallbackPrice,
          buyPrice: 0,
          sellPrice: 0,
          source: fallbackPrice > 0 ? 'global_average' : 'not_found',
          liquidity: 0,
        }
      }
    }
