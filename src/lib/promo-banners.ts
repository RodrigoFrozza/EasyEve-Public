...
/** Only http(s) URLs are allowed — an EXTERNAL_LINK action feeds this straight into
 *  window.open() client-side, so a `javascript:`/`data:` value would be a stored XSS. */
function assertSafeExternalUrl(rawUrl: unknown): void {
  if (typeof rawUrl !== 'string' || rawUrl.trim().length === 0) {
    throw new AppError(ErrorCodes.INVALID_INPUT, 'External link URL is required', 400)
  }
  let parsed: URL
  try {
    parsed = new URL(rawUrl)
  } catch {
    throw new AppError(ErrorCodes.INVALID_INPUT, 'External link URL must be a valid absolute URL', 400)
  }
  if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
    throw new AppError(ErrorCodes.INVALID_INPUT, 'External link URL must use http or https', 400)
  }
}
...
export function buildPromoBannerWriteData(input: PromoBannerInput): PromoBannerWriteData {
  ...
  if (input.actionType === CAMPAIGN_ACTIONS.EXTERNAL_LINK) {
    assertSafeExternalUrl((input.actionConfig as Record<string, unknown> | null)?.url)
  }
